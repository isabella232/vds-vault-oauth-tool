import os, webbrowser, logging, time, argparse, configparser, sys, json
...
def main():
        global logger, logFile, default_client_id, default_as_metadata_url, default_as_metadata_json, default_port, default_username, is_adfs
        import sys
        ...
        if ('oauth_connection' in config):
                ...
                if (config['oauth_connection']['log_directory'] == ""):                
                        logFile = projectPath + "\logs"
                        print("Output log directory: " + projectPath + "\logs\n")
                else:
                        logFile = config['oauth_connection']['log_directory'].strip("\\")
                        print("Output log directory: " + config['oauth_connection']['log_directory'])

        # Input parameters override the config.ini file.
        ...
        logFile = args.log_directory.strip() if args.log_directory != None and args.log_directory.strip() != "" else logFile
